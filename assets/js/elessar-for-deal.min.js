!function (definition) {
    ...
    return function e(t, n, r) {
        ...
        return s
                ...
                var Element = Base.extend({initialize: function (html) {
                        this.$el = $(html);
                        this.$data = {};
            ...
            }, {"./raf": 6, estira: 10}], 2: [function (_dereq_, module, exports) {
                var has = Object.prototype.hasOwnProperty;
                ...
                var Indicator = Element.extend(vertical).extend({initialize: function initialize(options) {
            ...
            }, {"./element": 1, "./vertical": 9}], 4: [function (_dereq_, module, exports) {
                var Element = _dereq_("./element.js");
                var Mark = Element.extend({initialize: function initialize(options) {
                ...
                module.exports = Mark
            }, {"./element.js": 1}], 5: [function (_dereq_, module, exports) {
                var Range = _dereq_("./range");
                var requestAnimationFrame = _dereq_("./raf");
                var Phantom = Range.extend({initialize: function initialize(options) {
                        initialize.super$.call(this, $.extend({readonly: true, label: "+"}, options));
            ...
            }, {"./raf": 6, "./range": 7}], 6: [function (_dereq_, module, exports) {
                var lastTime = 0;
                ...
                var vertical = _dereq_("./vertical");
                var Range = Element.extend(vertical).extend({initialize: function initialize(options) {
                        var self = this;
                        initialize.super$.call(this, '<div class="elessar-range"><div class="elessar-deal"><div class="discount-val text-center"></div><div class="de-wrap text-center"><h4>Discount</h4><br/><div class="input-group input-discount"><input type="text" name="input-dis" class="form-control rounded input-dis numbersOnly" /><span class="input-group-addon">%</span></div><br/><span class="time-range"></span></span></div></div><span class="elessar-barlabel elessar-barlabel-1">');
                        this.options = options;
                        this.perant = options.perant;
                        if (this.options.rangeClass)
                            this.$el.addClass(this.options.rangeClass);
                        if (!this.readonly()) {
                            this.$el.prepend('<div class="elessar-handle">').append('<span class="elessar-barlabel elessar-barlabel-2"></span><div class="elessar-handle">');
                        ...
                        if(text != '+'){
                            eval('var obj='+text);
                            var start_time = obj[0];
                            var end_time = obj[1];
                            
                            
                            this.$el.find(".elessar-barlabel-1")[this.options.htmlLabel ? "html" : "text"](start_time);
                            this.$el.find(".elessar-barlabel-2")[this.options.htmlLabel ? "html" : "text"](end_time);
                            this.$el.find(".time-range").text(start_time +' - '+ end_time);
                        }else
                        ...
                        valOpts = $.extend({}, {dontApplyDelta: false, trigger: true}, valOpts || {});
                        ...
                        this.draw(this.perant.options.vertical ? {top: start, minHeight: size} : {left: start, minWidth: size});
                        return this;
            ...
            }, {"./element": 1, "./eventprop": 2, "./vertical": 9}], 8: [function (_dereq_, module, exports) {
                var Element = _dereq_("./element");
                var Range = _dereq_("./range");
                ...
                var RangeBar = Element.extend(vertical).extend({initialize: function initialize(options) {
                        options = options || {};
                        ...
                        if (options.bgMark) {
                            ...
                            if (options.bgMark.count) {
                                for (var i = 0; i < options.bgMark.count; ++i) {
                                    this.$markContainer.append(new Mark({label: options.bgMark.label, value: i / options.bgMark.count, perant: this}).$el)
                                }
                                ...
                                for (var i = this.abnormalise(this.options.min); i < this.abnormalise(this.options.max); i += this.abnormalise(options.bgMark.interval)) {
                                    this.$markContainer.append(new Mark({label: options.bgMark.label, value: i, perant: this}).$el)
                                }
                            }
                        }
                        var self = this;
                        if (options.indicator) {
                            var indicator = this.indicator = new Indicator({perant: this, vertical: this.options.vertical, indicatorClass: options.indicatorClass});
                        ...
                        var $range = Range({perant: this, snap: this.options.snap ? this.abnormaliseRaw(this.options.snap + this.options.min) : null, label: this.options.label, rangeClass: this.options.rangeClass, minSize: this.options.minSize ? this.abnormaliseRaw(this.options.minSize + this.options.min) : null, readonly: this.options.readonly, htmlLabel: this.options.htmlLabel});
                        ...
                        ranges.forEach(function (range, i) {
                            ...
                            if (self.ranges[i]) {
                                ...
                                self.addRange(range.map($.proxy(self.abnormalise, self)),range[2]);
                            }
                        });
                        ...
                        if ((direct || phantom) && this.ranges.length < this.options.maxRanges && !$("body").is(".elessar-dragging, .elessar-resizing") && !this.readonly()) {
                            if (!this.phantom)
                                this.phantom = Phantom({perant: this, snap: this.options.snap ? this.abnormaliseRaw(this.options.snap + this.options.min) : null, label: "+", minSize: this.options.minSize ? this.abnormaliseRaw(this.options.minSize + this.options.min) : null, rangeClass: this.options.rangeClass});
                ...
                RangeBar.defaults = {min: 0, max: 100, valueFormat: function (a) {
                        return a
                    }, valueParse: function (a) {
                        return a
                    }, maxRanges: Infinity, readonly: false, bgLabels: 0, deleteTimeout: 5e3, allowDelete: false, vertical: false, htmlLabel: false, allowSwap: true};
                module.exports = RangeBar
            }, {"./element": 1, "./eventprop": 2, "./indicator": 3, "./mark.js": 4, "./phantom": 5, "./range": 7, "./vertical": 9}], 9: [function (_dereq_, module, exports) {
                module.exports = {isVertical: function () {
...
$(function(){
    $('.input-dis').keyup(function(){
        $(this).parents('.de-wrap').prev('.discount-val').empty().append(($(this).val() != '')?$(this).val()+'%':'');
    })
})
